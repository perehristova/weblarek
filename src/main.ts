...
events.on('cart:changed', () => { 
    ...
    const cardElements = items.map((item, index) => { 
        ...
        return card.render({ product: item, index: index }); 
    }); 
...
events.on('buyer:changed', (data: IBuyer) => {
    ...
    paymentForm.setErrors({ payment: errors.payment, address: errors.address });
    contactForm.setErrors({ email: errors.email, phone: errors.phone });
});
...
events.on('contact:form:submit', async () => { 
    try { 
        ...
        modal.close(); 
        successModal.data = { total: result.total }; 
